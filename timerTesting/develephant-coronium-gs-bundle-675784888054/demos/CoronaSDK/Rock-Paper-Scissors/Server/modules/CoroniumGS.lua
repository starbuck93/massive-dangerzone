...
function GS:onClientData( client_sock, data )
	...
	if data and self.showOutput == true then
		p( data )
	end
	...
	if client.pending_handshake then
		if data._handshook then
			if data.key == self.connection_key then
				...
				if data.data then
					client:_setPlayerData( data.data )
				end
...
-- searched by. See also `addToGameQueue` game criteria paramter.
-- @local  
function GS:createGame( initial_player, players_max, game_criteria )
	...
	local game = Game:new({
		...
		players_max = players_max or 2,
		game_criteria = game_criteria or nil,
		game_id = game_id,
...
function GS:addToGameQueue( player, players_max, game_criteria )
	...
	if game_criteria and game_criteria.world and players_max == 0 then
		...
		local qp = { player = player, players_max = players_max, game_criteria = game_criteria or nil }
...
function GS:_startQueueWatcher()
	...
	self.queue_timer = timer.setInterval( 5555, function() 
...
function GS:_checkGameQueue()
	...
	for game_id, game in pairs( self.games ) do
		...
		if game_state == 'open' then
			...
			for i=1, #self.client_game_queue do

				local qp = self.client_game_queue[ i ]

				if qp and ( qp.players_max == players_max ) then
					--== remove from queue, add to game
		...
		elseif game_state == 'closed' then
			self:_removeGame( game_id )
		end
	end
